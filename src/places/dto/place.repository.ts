...
import { GetPlacesFilterDto } from "./get-places-filter.dto";
...
export class PlacesRepository extends Repository<Place> {
    ...
    async getPlaces(filterDto: GetPlacesFilterDto): Promise<Place[]>{
        ...
        if(search){
            query.andWhere('LOWER(place.name) LIKE LOWER(:search) OR LOWER(place.description) LIKE LOWER(:search) OR LOWER(place.address) LIKE LOWER(:search)', {search: `%${search}%`},)
        }
